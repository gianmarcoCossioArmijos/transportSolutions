...
public class transportSolutionsModificarEgresoView extends javax.swing.JInternalFrame {

    List<Empleado> lista_empleado;

    public transportSolutionsModificarEgresoView() {
        initComponents();
        cargarEmpleado();
        deshabilitar();
    }
            ...
            if (txtDescripcion.getText().length() > 0) {
                if (txtMonto.getText().length() > 0) {
                    if (dcFecha.getDate().toString().length() > 0) {
                        if (!"SELECCIOANR".equals(cmbResponsable.getSelectedItem().toString())) {
    ...
    private void txtIdActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtIdActionPerformed
        ...
        if (txtId.getText().length() > 0) {
            ...
            EgresoBD ebd = new EgresoBD();
            DefaultTableModel tabla_temporal;
            tabla_temporal = ebd.buscarEgresoID(Integer.parseInt(txtId.getText()));

            if (tabla_temporal.getRowCount() > 0) {

                txtTipo.setText(tabla_temporal.getValueAt(0, 1).toString());
                txtDescripcion.setText(tabla_temporal.getValueAt(0, 2).toString());
                txtMonto.setText(tabla_temporal.getValueAt(0, 3).toString());
                ...
                cmbResponsable.setSelectedItem(tabla_temporal.getValueAt(0, 5).toString());
                txtComprobante.setText(tabla_temporal.getValueAt(0, 6).toString());

                habilitar();
            } else {
                JOptionPane op = new JOptionPane("Error al buscar ID ");
                op.setMessageType(JOptionPane.WARNING_MESSAGE);
            }
        } else {
            JOptionPane op = new JOptionPane("Debe ingresar ID");
