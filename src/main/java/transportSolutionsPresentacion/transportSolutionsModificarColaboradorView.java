...
public class transportSolutionsModificarColaboradorView extends javax.swing.JInternalFrame {

    List<AreaTrabajo> lista_area;
    List<AreaTrabajo> lista_puesto;

    public transportSolutionsModificarColaboradorView() {
        initComponents();
        cargarAreaTrabajo();
        cargarPuestoTrabajo();
    ...
    void habilitar() {
        ...
        txtEntidadFinanciera.setEnabled(true);
        txtCuentaBancaria.setEnabled(true);
        cmbAreaTrabajo.setEnabled(true);
        cmbPuesto.setEnabled(true);
        cmbSistemaPensiones.setEnabled(true);
    }
    ...
    void deshabilitar() {
        ...
        txtEntidadFinanciera.setEnabled(false);
        txtCuentaBancaria.setEnabled(false);
        cmbAreaTrabajo.setEnabled(false);
        cmbPuesto.setEnabled(false);
        cmbSistemaPensiones.setEnabled(false);
    }
    ...
    private void initComponents() {
        ...
        setTitle("MODIFICAR COLABORADOR");
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/mostrar_empleado.png"))); // NOI18N
                                ...
                                if (!"SELECCIONAR".equals(cmbSistemaPensiones.getSelectedItem().toString())) {
                                    if (txtEntidadFinanciera.getText().length() > 0) {
                                        if (txtCuentaBancaria.getText().length() > 0) {
                                            ...
                                            int id = Integer.parseInt(ebd.buscarIdEmpleado(txtDni.getText()));
                                            ebd.modificarEmpleado(e, id);
                                            JOptionPane op = new JOptionPane("empleado modificado");
    ...
    private void txtDniActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtDniActionPerformed
        ...
        if (txtDni.getText().length() > 0) {
            ...
            if (tabla_temporal.getRowCount() > 0) {
                ...
                txtSueldo.setText(tabla_temporal.getValueAt(0, 7).toString());
                txtEntidadFinanciera.setText(tabla_temporal.getValueAt(0, 9).toString());
                txtCuentaBancaria.setText(tabla_temporal.getValueAt(0, 10).toString());

                String sp = tabla_temporal.getValueAt(0, 8).toString();
                if (sp.equals("SNP")) {
    ...
    private void cmbSistemaPensionesItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cmbSistemaPensionesItemStateChanged
        ...
        if (cmbSistemaPensiones.getSelectedObjects().toString().equals("AFP")) {
